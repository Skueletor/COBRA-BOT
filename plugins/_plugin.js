...
cobra.addCommand({pattern: 'install ?(.*)', fromMe: true, desc: Lang.INSTALL_DESC, warn: Lang.WARN, dontAddCommandList: true}, (async (message, match) => {

    if (match[1] == '') return await message.client.sendMessage(message.jid,Lang.NEED_URL + '.install https://gist.github.com/sisula/4232b1c8c4734e1f06c3d991149c6fbd', MessageType.text)
    try {
    ...
    if (response.statusCode == 200) {
        ...
        try {
            ...
            fs.unlinkSync('/root/wabotcobra/plugins/' + plugin_name + '.js')
        ...
        if (find.includes(plugin_name)) {
        ...
        else if (response.body.includes('formation') && !match[1].includes('sisula')) {
            ...
            if (!match[1].includes('sisula') && DEG.level > 99) {
                await message.client.sendMessage(message.jid,LANG.limit + DEG.level + '_', MessageType.text)
                fs.unlinkSync('/root/wabotcobra/plugins/' + plugin_name + '.js')
            }
            else if (!match[1].includes('sisula') && DEG.level < 100) {
...
cobra.addCommand({pattern: 'plugin$', fromMe: true, desc: Lang.PLUGIN_DESC, dontAddCommandList: true}, (async (message, match) => {
    ...
    if (plugins.length < 1) {
        ...
        plugins.map(
            (plugin) => {
                let vf = plugin.dataValues.url.includes('sisula') ? msg : unmsg
...
cobra.addCommand({pattern: 'remove(?: |$)(.*)', fromMe: true, desc: Lang.REMOVE_DESC, dontAddCommandList: true}, (async (message, match) => {
